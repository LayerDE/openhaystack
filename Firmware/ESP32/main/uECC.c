...
const int sizeofCurve = sizeof(struct uECC_Curve_t);
...
#if !asm_clear
#define uECC_vli_clear(vli, words) memset(vli, 0, words * sizeof(uECC_word_t))
#endif /* !asm_clear */
...
/* Sets dest = src. */
#if !asm_set
#define uECC_vli_set(dest,src,words) memcpy(dest, src, words * sizeof(uECC_word_t))
#endif /* !asm_set */
...
uECC_VLI_API cmpresult_t uECC_vli_cmp(const uECC_word_t* left,
    const uECC_word_t* right,
    wordcount_t num_words) {
    uECC_word_t* tmp = malloc(num_words*uECC_WORD_SIZE);
...
uECC_VLI_API void uECC_vli_mmod(uECC_word_t* result,
    ...
    const uECC_word_t* mod,
    wordcount_t num_words) {
    uECC_word_t* mod_multiple = malloc(num_words * uECC_WORD_SIZE *2);
    uECC_word_t* tmp = malloc(num_words * uECC_WORD_SIZE *2);
...
uECC_VLI_API void uECC_vli_modInv(uECC_word_t* result,
    ...
    const uECC_word_t* mod,
    wordcount_t num_words) {
    uECC_word_t* a = malloc(num_words * uECC_WORD_SIZE),
        *b = malloc(num_words * uECC_WORD_SIZE),
        *u = malloc(num_words * uECC_WORD_SIZE),
        *v = malloc(num_words * uECC_WORD_SIZE);
...
static void apply_z(uECC_word_t* X1,
    ...
    uECC_Curve curve) {
    uECC_word_t *t1 = malloc(curve->num_words * uECC_WORD_SIZE);
...
static void XYcZ_initial_double(uECC_word_t* X1,
    ...
    uECC_Curve curve) {
    uECC_word_t *z = malloc(curve->num_words * uECC_WORD_SIZE);
...
static void XYcZ_add(uECC_word_t* X1,
    ...
    uECC_word_t *t5 = malloc(curve->num_words * uECC_WORD_SIZE);
    wordcount_t num_words = curve->num_words;
...
static void XYcZ_addC(uECC_word_t* X1,
    ...
    /* t1 = X1, t2 = Y1, t3 = X2, t4 = Y2 */
    uECC_word_t *t5 = malloc(curve->num_words * uECC_WORD_SIZE),
        *t6 = malloc(curve->num_words * uECC_WORD_SIZE),
        *t7 = malloc(curve->num_words * uECC_WORD_SIZE);
    wordcount_t num_words = curve->num_words;
...
static void EccPoint_mult(uECC_word_t* result,
    ...
    uECC_Curve curve) {
    /* R0 and R1 */
    uECC_word_t *Rx[2] = {malloc(curve->num_words * uECC_WORD_SIZE),
                        malloc(curve->num_words * uECC_WORD_SIZE)};
    uECC_word_t *Ry[2] = {malloc(curve->num_words * uECC_WORD_SIZE),
                        malloc(curve->num_words * uECC_WORD_SIZE)};
    uECC_word_t *z = malloc(curve->num_words * uECC_WORD_SIZE);
    bitcount_t i;
    uECC_word_t nb;
...
static uECC_word_t EccPoint_compute_public_key(uECC_word_t* result,
    ...
    uECC_Curve curve) {
    uECC_word_t *tmp1 = malloc(curve->num_words * uECC_WORD_SIZE);
    uECC_word_t *tmp2 = malloc(curve->num_words * uECC_WORD_SIZE);
...
uECC_VLI_API int uECC_generate_random_int(uECC_word_t* random,
    ...
    wordcount_t num_words, uECC_RNG_Function g_rng_function) {
...
#else
    uECC_word_t *_private = malloc(curve->num_words * uECC_WORD_SIZE);
    uECC_word_t *_public = malloc(curve->num_words * uECC_WORD_SIZE * 2);
#endif
    uECC_word_t tries;

    for (tries = 0; tries < uECC_RNG_MAX_TRIES; ++tries) {
        if (!uECC_generate_random_int(_private, curve->n, BITS_TO_WORDS(curve->num_n_bits), g_rng_function)) {
#if uECC_VLI_NATIVE_LITTLE_ENDIAN == 0
            free(_private, _public);
#endif
            return 0;
        }
...
int uECC_shared_secret(const uint8_t* public_key,
    ...
    uECC_word_t *tmp = malloc(curve->num_words * uECC_WORD_SIZE);
    ...
    if (g_rng_function) {
        if (!uECC_generate_random_int(p2[carry], curve->p, num_words, g_rng_function)) {
            return 0;
        }
        initial_Z = p2[carry];
    }
...
#else
    uECC_word_t* point = malloc(curve->num_words * uECC_WORD_SIZE * 2);
#endif
...
int uECC_valid_point(const uECC_word_t* point, uECC_Curve curve) {
    ...
    ret_val = (int)(uECC_vli_equal(tmp1, tmp2, num_words));
    free(tmp1);
    free(tmp2);
...
#endif
    int ret_val = uECC_valid_point(_public, curve);
#if uECC_VLI_NATIVE_LITTLE_ENDIAN == 0
    free(_public);
#endif
    return ret_val;
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    memcpy((uint8_t*)native, bits, bits_size);
#else
...
static int uECC_sign_with_k(const uint8_t* private_key,
    ...
    uECC_word_t *tmp = malloc(curve->num_words * uECC_WORD_SIZE);
    uECC_word_t *s = malloc(curve->num_words * uECC_WORD_SIZE);
    uECC_word_t* k2[2] = { tmp, s };
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    uECC_word_t* p = (uECC_word_t*)signature;
#else
    uECC_word_t *p = malloc(curve->num_words * uECC_WORD_SIZE * 2);
#endif
    uECC_word_t carry;
    wordcount_t num_words = curve->num_words;
    wordcount_t num_n_words = BITS_TO_WORDS(curve->num_n_bits);
    ...
    else if (!uECC_generate_random_int(tmp, curve->n, num_n_words, g_rng_function)) {
        free(tmp);
        free(s);
#if uECC_VLI_NATIVE_LITTLE_ENDIAN == 0
        free(p);
...
int uECC_sign(const uint8_t* private_key,
    ...
    uint8_t* signature,
    uECC_Curve curve, uECC_RNG_Function g_rng_function) {
    uECC_word_t *k = malloc(curve->num_words * uECC_WORD_SIZE);
    uECC_word_t tries;

    for (tries = 0; tries < uECC_RNG_MAX_TRIES; ++tries) {
        if (!uECC_generate_random_int(k, curve->n, BITS_TO_WORDS(curve->num_n_bits),g_rng_function)) {
            free(k);
            return 0;
        }

        if (uECC_sign_with_k(private_key, message_hash, hash_size, k, signature, curve, g_rng_function)) {
            free(k);
            return 1;
        }
    }
...
int uECC_sign_deterministic(const uint8_t* private_key,
    ...
    for (tries = 0; tries < uECC_RNG_MAX_TRIES; ++tries) {
        uECC_word_t *T = malloc(curve->num_words * uECC_WORD_SIZE);
    ...
    filled:
        ...
        if (uECC_sign_with_k(private_key, message_hash, hash_size, T, signature, curve, g_rng_function)) {
            free(T);
            return 1;
        }
...
int uECC_verify(const uint8_t* public_key,
    ...
    uECC_Curve curve) {
    uECC_word_t *u1 = malloc(curve->num_words * uECC_WORD_SIZE), *u2 = malloc(curve->num_words * uECC_WORD_SIZE);
    uECC_word_t *z = malloc(curve->num_words * uECC_WORD_SIZE);
    uECC_word_t *sum = malloc(curve->num_words * uECC_WORD_SIZE * 2);
    uECC_word_t *rx = malloc(curve->num_words * uECC_WORD_SIZE);
    uECC_word_t *ry = malloc(curve->num_words * uECC_WORD_SIZE);
    uECC_word_t *tx = malloc(curve->num_words * uECC_WORD_SIZE);
    uECC_word_t *ty = malloc(curve->num_words * uECC_WORD_SIZE);
    uECC_word_t *tz = malloc(curve->num_words * uECC_WORD_SIZE);
    ...
    uECC_word_t _public =  = malloc(curve->num_words * uECC_WORD_SIZE * 2);
#endif    
    uECC_word_t *r = malloc(curve->num_words * uECC_WORD_SIZE), *s = malloc(curve->num_words * uECC_WORD_SIZE);
    wordcount_t num_words = curve->num_words;
    wordcount_t num_n_words = BITS_TO_WORDS(curve->num_n_bits);
    ...
    /* Accept only if v == r. */
    int ret_val = (int)(uECC_vli_equal(rx, r, num_words));
    free(u1);
    free(u2);
    free(z);
    free(sum);
